...
public class DAO extends DBContext {
    ...
    public User getUserByUsername(String username) {
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                return new User(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getInt(5), rs.getFloat(6), rs.getFloat(7), rs.getString(8), rs.getInt(9), rs.getInt(10), rs.getString(11));
            }
        } catch (Exception e) {
        }
        return null;
    ...
    public void insertUser(String username, String password, String email, float accountBalance, float totalMoney, float discount, String avatar, int role, int status, String timeCreated) {
        try {
            PreparedStatement ps = getConnection().prepareStatement("INSERT INTO [User] VALUES(?,?,?,?,?,?,?,?,?,?)");
            ps.setString(1, username);
            ps.setString(2, password);
            ps.setString(3, email);
            ps.setFloat(4, accountBalance);
            ps.setFloat(5, totalMoney);
    ...
    public List<Supplier> getSupplierByCardTypeID(int CardTypeID) {
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new Supplier(rs.getInt(1), rs.getString(2), new CardType(rs.getInt(3)), rs.getString(4)));
            }
        } catch (Exception e) {
        }
        return list;
    ...
    public static void main(String[] args) {
        System.out.println(new DecimalFormat("###,###,###").format(new DAO().getPriceBySupplierID(1).get(0).getPrice()));
    }
